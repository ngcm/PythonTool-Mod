 ...
 * (mbe30_inventory_basic/ContainerBasic.java)
 * 
 * Original code by:
 * User: brandon3055
 * Date: 06/01/2015
...
public class ContainerComputerBlock extends Container {
	...
	//  36 - 62 = TileInventory slots, which map to our TileEntity slot numbers 0 - 26)
	...
	public ContainerComputerBlock(InventoryPlayer invPlayer, TileEntityComputerBlock tileEntityComputerBlock) {
		...
		if (TE_INVENTORY_SLOT_COUNT != tileEntityComputerBlock.getSizeInventory()) {
			System.err.println("Mismatched slot count in ContainerBasic(" + TE_INVENTORY_SLOT_COUNT
					+ ") and TileInventory (" + tileEntityComputerBlock.getSizeInventory()+")");
		}
		...
		for (int y = 0; y < TE_ROW_COUNT; y++) {
			for (int x = 0; x < TE_COLUMN_COUNT; x++) {
	...
	{
		return tileEntityComputerBlock.isUseableByPlayer(player);
	}
	...
	public void onContainerClosed(EntityPlayer playerIn)
	{
		...
		this.tileEntityComputerBlock.closeInventory(playerIn);
	}
}
